...
function editarCampo(campoId){
  ...
  if (campoId === 'mcEmail') {
    ...
    campo.removeEventListener('blur', campo._blurHandler);
    
    // Cria novos handlers
    campo._inputHandler = function() {
      validarCampoMinhaConta('mcEmail', this.value);
    };
    
    campo._blurHandler = function() {
      validarCampoMinhaConta('mcEmail', this.value);
    };
    
    // Adiciona os novos event listeners
    campo.addEventListener('input', campo._inputHandler);
    campo.addEventListener('blur', campo._blurHandler);
  }
}

// Função para validar campo específico
function validarCampoMinhaConta(campoId, valor){
  console.log(`🔍 validarCampoMinhaConta chamada para ${campoId} com valor: "${valor}"`);
  ...
  console.log(`📊 Resultado: válido=${valido}, mensagem="${mensagem}"`);
  ...
  if(errorDiv) {
    if(!valido && valor.trim()) {
      ...
      console.log(`🔴 Mostrando erro para ${campoId}: ${mensagem}`);
    } else {
      // Esconde a mensagem de erro quando o campo é válido OU está vazio
      errorDiv.classList.remove('show');
      errorDiv.style.display = 'none'; // Garante que o erro seja ocultado
      errorDiv.textContent = '';
      console.log(`✅ Ocultando erro para ${campoId} - Campo válido ou vazio`);
    }
  } else {
    console.log(`❌ Elemento de erro não encontrado: ${campoId}Error`);
  }
